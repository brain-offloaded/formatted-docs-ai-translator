...
-   **분산된 비즈니스 로직**: 캐시 태그 등록 여부 확인, 번역 결과 요약, ZIP 다운로드와 같은 도메인 규칙이 각 컴포넌트와 훅에 분산되어 있습니다. 이로 인해 백엔드 API가 변경될 때 여러 파일을 동시에 수정해야 하는 문제가 발생합니다. 공통 서비스를 통한 로직 중앙화가 필요합니다.
